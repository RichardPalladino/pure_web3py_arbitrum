...
IPC_PATH = "/home/arbitrum/data/arbitrum/arbitrum.ipc"  # must have permission to access, read/write
...
def get_deployed_factories(w3) -> dict:
    ...
    # load factory dict
    with open("stable_dexs.json", "r") as dex_file:
...
def get_pool_data(_address, _factory_data, w3_provider) -> dict:
    ...
    try:
        ...
        print(f"Could not set ABI for fee_type {_factory_data['fee_type']}")
...
def main() -> None:
    ...
    for factory_address, factory_values in factories.items():
        ...
        for tmp_lp_address in factory_pools:
            if (tmp_lp_address in pairs.keys()) and (pairs[tmp_lp_address] == False):
                continue
            else:
                ## Get dict of pool data, if tmp_lp_address isn't bogus, i.e., has been made one of my addresses
                tmp_result = get_pool_data(
                    tmp_lp_address,
                    factories[factory_address],
                    ipc,
                )  # factories is a dict of factory_address -> factory values (num_pools, router, fee_type, solidly, etc.)
                if tmp_result["stable_pool"] == True:
                    print(f"{tmp_lp_address} is a stable pool")
                    pairs[tmp_lp_address] = tmp_result
                else:
            ...
            if (pairs[tmp_lp_address] is None) or (pairs[tmp_lp_address] == False):
                ...
                if (
                    ...
                    adj_reserve0 = pairs[tmp_lp_address]["reserves"][0] / (
                        10 ** pairs[tmp_lp_address]["token0"]["decimals"]
                    )
                    adj_reserve1 = pairs[tmp_lp_address]["reserves"][1] / (
                        10 ** pairs[tmp_lp_address]["token1"]["decimals"]
                    )
                    print(
                        f"Adjusted reserve0: {adj_reserv0}, adjusted reserve1: {adj_reserve1}"
                    )
                    if (adj_reserve0 < 1) or (adj_reserve1 < 1):
                        ...
                        reserve_ratio = adj_reserve0 / adj_reserve1
                        print(f"Reserve ratio: {reserve_ratio}")
                        if (reserve_ratio > 2) or (reserve_ratio < 0.5):
                            print(f"Found unbalanced pair: {tmp_lp_address}")
                            pool_ticker += 1
                            factory_lps[factory_address].append(tmp_lp_address)
                        else:
                            print("Reserve ratio not effective")
                            pairs[tmp_lp_address] = False
